...
import { PaymentOptionsModal } from "@/components/ClientPaymentStages";
import {
  DollarSign,
  Download,
  Eye,
      ...
      <div className="space-y-6">
                  ...
                  <TableBody>
                    {invoiceList.map((invoice: Invoice) => (
                      <TableRow key={invoice.id}>
                        <TableCell className="font-medium">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">
                              {invoice.description || (invoice.type === 'stage_payment' ? `Etapa: ${invoice.stageName}` : 'Servicio General')}
                            </div>
                            {invoice.sifenCDC && (
                        ...
                        <TableCell>
                          <div>
                            <div className="font-medium">{invoice.projectName || 'Sin Proyecto'}</div>
        ...
        {paymentInvoice && (
          <PaymentOptionsModal 
            ...
            amount={typeof paymentInvoice.amount === 'string' ? parseFloat(paymentInvoice.amount) : paymentInvoice.amount}
            stageName={paymentInvoice.description || paymentInvoice.stageName || "Pago de Factura"}
            stageId={paymentInvoice.id}
            projectName={paymentInvoice.projectName || "Servicio General"}
            isInvoice={true}
          />
        )}
      </div>
